class DeferredTaskRunner
  ...
  def run_round
    DB.open(transaction: false) do |db|
      tasks_by_type = {}
      ...
      db[:deferred_task_running].multi_insert(
        all_tasks.map {|task|
          {
            task_id: task[:id],
      ...
      db[:deferred_task].filter(:id => all_tasks.map {|task| task[:id]}).update(:status => RUNNING_STATUS)
      ...
      watchdog_thread = Thread.new do
        DB.open(transaction: false) do |db|
          while watchdog_running.get
            ...
            db[:deferred_task_running].filter(:task_id => all_tasks.map {|task| task[:id]}).update(:last_checked_time => now)
            sleep 5
          end
        end
      end
      ...
      tasks_by_type.each do |type, tasks|
        begin
          ...
          db[:deferred_task].filter(:id => task_results.select(&:success?).map(&:id)).delete

          # Decrement retry count for those who failed
          db[:deferred_task].filter(:id => task_results.map(&:id)).update(:retries_remaining => :retries_remaining - Sequel.expr(1))
      ...
      db[:deferred_task_running].filter(:task_id => tasks_by_type.values.flatten.map {|task| task[:id]}).delete
    end
  end
